    ...
    $(function () {
        $("#datechoose").datepicker({
            ...
            onChangeMonthYear: function(year, month, format) {
                location.href = "JiaGeFenXi.aspx?date=" + year + "-" + fullmonth(month) + "-" + "<%=DateTime.Parse(Session["TodayDate"].ToString()).ToString("dd") %>";
            }
        });
        ...
        function log(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                $(".maindiv table").append("<tr><td>" + data[i].ItemType + "</td><td>" + data[i].ItemName + "</td><td>" + data[i].ItemBuyDate + "</td><td class='cellprice'>￥" + data[i].ItemPrice + "</td><td><a href='JiaGeFenXiMingXi.aspx?itemName=" + data[i].ItemName + "&itemType=" + data[i].ItemTypeValue + "'>查看详情</a></td></tr>");
            }
        }        
        function loading() {
            load = true;
    ...
    <div class="maindiv">
        ...
        <table cellspacing="0" border="1" style="width:100%;" class="tablelist">
            ...
            <tr>
                ...
                <td><a href="JiaGeFenXiMingXi.aspx?itemName=<%# Eval("ItemName") %>&itemType=<%# Eval("ItemTypeValue") %>">查看详细</a></td>
            </tr>
            </ItemTemplate>
            <FooterTemplate>
            <tr>
