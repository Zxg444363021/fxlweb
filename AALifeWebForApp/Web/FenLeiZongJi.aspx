<%@ Page Title="消费分类排行" Language="C#" MasterPageFile="UserControl/MasterPage.master" AutoEventWireup="true" CodeFile="FenLeiZongJi.aspx.cs" Inherits="FenLeiZongJi" %>
...
<div id="content">
    ...
    <div class="maindiv">
        ...
        <table cellspacing="0" border="1" style="width:100%;" class="tablelist">
            <tr>
                <th style="width:25%;">预警<a href='javascript:;' title='灰空值，蓝正常，黄临界，红超出'>？</a></th>
                <th style="width:25%;">类别名称</th>
                <th style="width:25%;" class="cellprice">总价格</th>
                <th style="width:25%;">操作</th>
            </tr>
            <asp:Repeater ID="List" runat="server">
            <ItemTemplate>
            <tr>
                <td><%# GetCategoryDen(Double.Parse(Eval("CategoryTypePrice").ToString()), Double.Parse(Eval("ItemPriceTotal").ToString())) %></td>
                <td><%# Eval("CategoryTypeName").ToString() == "" ? "未知" : Eval("CategoryTypeName")%></td>
                <td class="cellprice">￥<%# Eval("ItemPriceTotal", "{0:0.0##}") %></td>
                <td><a href="FenLeiZongJiMingXi.aspx?catTypeId=<%# Eval("CategoryTypeID") %>&catTypeName=<%# Eval("CategoryTypeName").ToString() == "" ? "未知" : Eval("CategoryTypeName")%>">查看详细</a></td>
            </tr>
            </ItemTemplate>
            <FooterTemplate>
            <tr>
                <td colspan="4" class="noitemcell"><asp:Label ID="Label1" runat="server" Text="没有消费记录。" Visible='<%# List.Items.Count == 0 %>'></asp:Label></td>
            </tr>
            </FooterTemplate>
            </asp:Repeater>
            <tr class="totalcell">
