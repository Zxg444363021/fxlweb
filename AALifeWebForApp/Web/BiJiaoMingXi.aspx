...
<script type="text/javascript">
    ...
    function chart_click(index) {
        location.href = "ItemNumDetail.aspx?url=<%=Server.UrlEncode(Request.Url.ToString()) %>&" + arrChart[index] + "&date=<%=Session["TodayDate"].ToString() %>";
    }
    function chart_click2(index) {
        location.href = "ItemNumDetail.aspx?url=<%=Server.UrlEncode(Request.Url.ToString()) %>&" + arrChart2[index] + "&date=<%=DateTime.Parse(Session["TodayDate"].ToString()).AddMonths(-1).ToString("yyyy-MM-dd") %>";
    }
</script>
</asp:Content>
...
<div id="content">
    ...
    <table cellspacing="0" border="0" style="width:100%;" class="tabledate">
        <tr>
            ...
            <td style="width:20%;"><strong>日期：</strong><%=DateTime.Parse(Session["TodayDate"].ToString()).ToString("yyyy年MM月") %></td>
    ...
    <div class="maindiv">
        ...
        <table cellspacing="0" border="1" style="width:100%;" class="tablelist">
            <tr>
                ...
                <th style="width:20%;" class="bjcur">本月次数 <img src="/Images/Others/zim.gif" border="0" alt="此列可点击" title="此列可点击" /></th>
                <th style="width:20%;" class="cellprice bjcur">本月总价</th>
                <th style="width:20%;" class="bjprev">上月次数 <img src="/Images/Others/zim.gif" border="0" alt="此列可点击" title="此列可点击" /></th>
                <th style="width:20%;" class="cellprice bjprev">上月总价</th>
            </tr>
            <asp:Repeater ID="CatTypeList" runat="server">
            <ItemTemplate>
            <tr>
                <td><%# Eval("ItemName") %></td>
                <td class="bjcur"><a href="ItemNumDetail.aspx?url=<%=Server.UrlEncode(Request.Url.ToString()) %>&itemName=<%# Eval("ItemName") %>&itemType=<%# Eval("ItemTypeCur") %>&catTypeId=<%# Eval("CatTypeIDCur") %>&date=<%# Session["TodayDate"].ToString() %>"><%# Eval("CountNumCur")%> 次 </a></td>
                <td class="cellprice bjcur">￥<%# Eval("ItemPriceCur", "{0:0.0##}")%></td>
                <td class="bjprev"><a href="ItemNumDetail.aspx?url=<%=Server.UrlEncode(Request.Url.ToString()) %>&itemName=<%# Eval("ItemName") %>&itemType=<%# Eval("ItemTypePrev") %>&catTypeId=<%# Eval("CatTypeIDPrev") %>&date=<%# DateTime.Parse(Session["TodayDate"].ToString()).AddMonths(-1).ToString("yyyy-MM-dd") %>"><%# Eval("CountNumPrev")%> 次 </a></td>
                <td class="cellprice bjprev">￥<%# Eval("ItemPricePrev", "{0:0.0##}")%></td>
            </tr>
            </ItemTemplate>
            <FooterTemplate>
            <tr>
                <td colspan="5" class="noitemcell"><asp:Label ID="Label1" runat="server" Text="没有消费记录。" Visible="<%# CatTypeList.Items.Count == 0 %>"></asp:Label></td>
            </tr>
            </FooterTemplate>
            </asp:Repeater>
        </table>
