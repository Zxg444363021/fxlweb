    ...
    $(function () {
        $("#datechoose").datepicker({
            ...
            onChangeMonthYear: function(year, month, format) {
                location.href = "TianShuFenXi.aspx?date=" + year + "-" + fullmonth(month) + "-" + "<%=DateTime.Parse(Session["TodayDate"].ToString()).ToString("dd") %>";
            }
        });
        ...
        function log(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                $(".maindiv table").append("<tr><td>" + data[i].ItemType + "</td><td>" + data[i].ItemName + "</td><td>" + data[i].ItemBuyDate + "</td><td>" + data[i].NotBuy + " 天前购买</td><td><a href='ItemList.aspx?date=" + data[i].ItemBuyDate + "'>查看详情</a></td></tr>");
            }
        }        
        function loading() {
            load = true;
