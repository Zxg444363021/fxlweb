...
public partial class AdminBackupData : AdminPage
{
    ...
    protected void Button2_Command(object sender, CommandEventArgs e)
    {
        ...
        BackupHelper.WriteBackupFile(GetFilePathName(fileName), sb.ToString());

        DownBackFile(fileName);
    }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        UserTableBLL user_bll = new UserTableBLL();
        UserInfo user = user_bll.GetUserByUserId(userId);
        string fileName = "aalife2.bak";
        ...
        if (itemListTab.Rows.Count > 0)
        {
            ...
            for (int i = itemListTab.Rows.Count - 1; i >= 0; i--)
            {
                ...
                sb.AppendLine("INSERT INTO ItemTable (ItemWebID, ItemType, ItemName, ItemPrice, ItemBuyDate, CategoryID, Recommend, Synchronize, RegionID, RegionType, ZhuanTiID, CardID) VALUES ('" +
                                dr["ItemID"].ToString() + "', '" +
                                dr["ItemType"].ToString() + "', '" +
                                Utility.ReplaceSql(dr["ItemName"].ToString()) + "', '" +
                                dr["ItemPrice"].ToString() + "', '" +
        ...
        if (zhangTab.Rows.Count > 0)
        {
            ...
            foreach (DataRow dr in zhangTab.Rows)
            {
                sb.AppendLine("INSERT INTO ZhuanZhangTable (ZZID, ZhangFrom, ZhangTo, ZhangMoney, ZhangDate, ZhangNote, ZhangLive, Synchronize) VALUES ('" +
                                ...
                                dr["ZhuanZhangTo"].ToString() + "', '" +
                                dr["ZhuanZhangMoney"].ToString() + "', '" +
                                String.Format("{0:yyyy-MM-dd}", dr["ZhuanZhangDate"].ToString()) + "', '" +
                                Utility.ReplaceSql(dr["ZhuanZhangNote"].ToString()) + "', '" +
    ...
    //下载备份
    private void DownBackFile(string fileName)
    {
        FileStream fileStream = new FileStream(GetFilePathName(fileName), FileMode.Open);
    ...
    {
        string filePath = Server.MapPath("Backup") + System.IO.Path.DirectorySeparatorChar;
